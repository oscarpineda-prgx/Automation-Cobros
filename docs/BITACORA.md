...
### 2026-09-15 05:14:35 — [DATO] Corrida completa 318949 Lacteos y Derivados (2025 + cierre 2026Q1)

Corrida aislada punta a punta pedida por Oscar: descarga CPA -> parquet -> Compras por mes -> Validacion. RFC CLD0507145H6, periodo 2025-01-01..2026-03-31 (convencion reunion 008 para un proveedor '2025'). 6,429,670 renglones, GIGANTE, partido POR MES (365k-476k por mes, ninguno desborda hoja; por trimestre habria dado archivos de ~1.4M con hojas de continuacion). VERIFICADO contra SQL: los 15 archivos cuadran al renglon, total 6,429,670 exacto. Descarga nueva request 671844 (215 MB, 5,277,621 filas, 8 CSV) con 2025 completo + enero 2026; la dedup por cobertura de _CTE_ELEGIDO la eligio sobre la vieja 618477 (solo 2025) y los soportes copiaron solo la nueva, tal como estaba previsto. Validacion: 1,030,630 folios auditables, 19,730 con diferencia > 1.0; reporte consolidado actualizado. HALLAZGO PRINCIPAL - octubre 2025 cruza solo 35%: NO es falla del cruce ni de CPA, es el dato de origen. 283,414 de 441,020 renglones de octubre (64.3%) vienen SIN numero de factura (invnbr vacio) en SQL, y la llave del cruce es codigo de barras + factura. Septiembre igual pero mas leve: 57,675 sin factura (13.5%). Medido tambien noviembre como control: solo 85 renglones sin factura y 100% de las facturas halladas en CPA. Cuando la factura SI viene, aparece en CPA practicamente siempre. Conviene reportarlo a Monica: esos renglones son inauditables por falta de folio en origen, no por cobertura de CPA. Febrero y marzo 2026 salen sin cruce (99.9% pendientes) POR DISENO: CPA se descarga solo hasta enero 2026 (reunion 008 decision 2) mientras Compras llega a marzo. ERROR PROPIO detectado y corregido: el vigilante que encadenaba las fases murio con UnicodeEncodeError al volcar el stdout del subproceso a una consola cp1252; revento DESPUES de que los Compras ya estaban escritos, asi que no se perdio trabajo, solo su salida en pantalla. La Validacion se relanzo aparte con PYTHONIOENCODING=utf-8.

---

### 2026-09-11 12:29:16 — [DECISIÓN] El auditor edita cto_aud/iva_aud/ieps_aud, no el bloque EDI (Monica)

CAMBIO DE REGLA DE NEGOCIO indicado por Monica via Oscar. Hasta hoy el flujo suponia que el auditor corregia el bloque EDI (ctonto_edi, poriva_edi, prieps_edi) y que de ahi se derivaban las columnas auditadas. NO ES ASI: el EDI es DATO DE ORIGEN —viene del sistema del cliente y de CPA Vision, el cruce rellena lo que puede y el resto se deja tal cual— y lo que el auditor corrige es el RESULTADO DE LA AUDITORIA: cto_aud, iva_aud e ieps_aud. De ahi hacia abajo (imp_aud, debio_pagar_ne, dif_det_ne, debio_pagar_inv, tot_pagado_inv, dif_det_inv) todo es resultado calculado y se recalcula SIEMPRE a partir de esas tres.

IMPLEMENTACION: recalculate_dataframe y prepare_compras_dataframe ganan el parametro respetar_auditadas. En False (por omision) las tres se DERIVAN del EDI con la regla de siempre —correcto cuando el DataFrame sale de SQL o del cruce, donde no hay criterio humano que preservar—; en True se RESPETA lo que traiga el archivo. Lo activan los dos unicos caminos que leen un Compras de disco, que es donde pudo pasar el auditor: recalculate_compras_file (paso 4) y write_validation_workbook (paso 5).

DOS REGLAS FINAS, las dos deliberadas: un 0 escrito a mano SE RESPETA —la presencia se mide por celda con dato y no por 'valor distinto de cero', porque un iva_aud=0 es una correccion legitima de producto exento—, y una celda BORRADA vuelve a la regla en vez de quedar en nulo, porque un NaN se propagaria a imp_aud y de ahi a la diferencia del folio.

BUG QUE ATRAPO LA PRUEBA END-TO-END, y que era mio: recalculate_compras_file preparaba respetando al auditor y despues llamaba a write_compras_workbook sin already_prepared, asi que el exportador VOLVIA A PREPARAR sin respetar y pisaba la correccion. El sintoma habria sido mudo: archivo bien formado, con los valores de la regla, como si el auditor no hubiera escrito nada. Corregido con already_prepared=True, que aqui no es una optimizacion sino lo que hace correcto el paso.

COLORES, que ahora dicen QUE SE HACE con cada columna: EDI en VERDE OSCURO (#375623, con texto blanco para que se lea) = dato de origen que no se edita; cto_aud/iva_aud/ieps_aud en PIEL (#FFF2CC) = lo que edita el auditor; imp_aud..dif_det_inv en VERDE CLARO (#E2F0D9) = resultado. Antes el piel estaba en el bloque EDI, o sea invitando a corregir justo lo que no se toca. La lista COLUMNAS_AUDITOR vive en calculations porque es regla de negocio; excel_exporter solo la pinta.

VERIFICADO: 9 casos de la semantica —la regla manda sin edicion, el valor del auditor manda con ella, el 0 a mano no se pisa, el IEPS 0.53 se respeta, imp_aud se recalcula desde lo editado, celda borrada vuelve a la regla, sin la bandera la regla pisa, y el bloque de resultado se recalcula siempre aunque se truque—; los tres grupos de color y el texto blanco sobre verde oscuro; y el CICLO REAL con SELECTA 741 (1,178 renglones, 1,118 cruzados): el paso 4 conserva el cto_aud editado y la Validacion pasa de 0.00 a 193,728.48.

Documentado en LOGICA_NEGOCIO 3.0.

**Archivos:** `automation_costos/calculations.py`, `automation_costos/recalculate.py`, `automation_costos/validation_exporter.py`, `automation_costos/excel_exporter.py`

---

### 2026-09-11 10:14:50 — [BUG] El paso 3 escribia un volcado, no un Compras: el paso 4 lo leia mal y perdia renglones

Lo destapo Oscar al notar que el archivo _EDI salia SIN FORMATO, con encabezados en blanco. No era cosmetico: era el sintoma de que ese archivo NO ES UN COMPRAS. El paso 3 (GUI) y el subcomando cpa-cruce escribian con DataFrame.to_excel(index=False) -hoja 'Sheet1', encabezados en la fila 1, sin formato-, pero recalculate.read_compras_workbook busca hojas 'Compras <anio>' y los ENCABEZADOS EN LA FILA 7. MEDIDO sobre un archivo de 10 renglones: el recalculo tomaba el renglon 6 como encabezado, devolvia 4 renglones y las columnas salian con nombres inventados (cnpj5, 5, vndname5). La cadena paso 3 -> paso 4 estaba ROTA y NO AVISABA: producia un archivo que parecia valido. Estaba documentado como pendiente desde el 2026-07-22 en CRUCE_IMPLEMENTACION 9 ('definir si el Excel de salida debe conservar el formato de la plantilla') y nadie habia recorrido 3->4->5 por separado; GENERAR TODO nunca paso por ahi porque cruza en memoria y escribe el Compras una sola vez. SOLUCION: nuevo excel_exporter.write_compras_cruzado(), PUNTO UNICO de escritura del cruce que comparten la GUI y el CLI para que no puedan volver a separarse; delega en write_compras_workbook y deduce el numero de proveedor del propio DataFrame (vndnbr) cuando no se le indica, porque quien cruza un archivo suelto no tiene por que volver a teclearlo. Efecto adicional deseable: al pasar por prepare_compras_dataframe, las columnas auditadas del _EDI ya reflejan los CFDI recien cruzados, en vez de quedarse con los valores previos. SEGUNDO CAMBIO, pedido por Oscar en la misma sesion: el paso a paso ahora produce la MISMA ESTRUCTURA que GENERAR TODO. El paso 1 escribe dentro de <salida>/<numero>_<nombre>/ (_ruta_en_carpeta_proveedor) y los pasos 3 y 4 nacen JUNTO A SU ORIGEN (_junto_a), asi que los cinco pasos quedan en la carpeta del proveedor. Antes escribian sueltos en la raiz de la carpeta de salida y el mismo proveedor terminaba repartido en dos sitios segun por que camino se hubiera trabajado. Si el nombre no permite deducir el proveedor se cae a la raiz: preferible escribir suelto que inventar una carpeta con nombre inventado. VERIFICADO con el ciclo REAL completo sobre SELECTA 741 (2025-01-01..2025-03-31, 1,178 renglones desde SQL): el _EDI se relee COMPLETO -1,178 de 1,178, era lo roto-, los encabezados se leen bien, trae hojas 'Compras 2025' y 'Pendientes_EDI', el cruce conserva los renglones (1,118 cruzados de 1,178), el recalculado tambien, y la Validacion sale con Resumen/Consolidado/Detalle PAGOS. Y sobre la ventana real: los cinco pasos caen en la misma carpeta del proveedor, y un nombre no reconocible se queda en la raiz sin inventar carpetas.

**Archivos:** `automation_costos/excel_exporter.py`, `automation_costos/app.py`, `main.py`

---

### 2026-09-11 08:11:00 — [BUG] El .exe no abria: PyInstaller dejo fuera las extensiones C de Pillow

Oscar probo el .exe del 2026-09-10 en una maquina virtual de auditor y no abrio: 'Failed to execute script main - cannot import name _imaging from PIL', con el traceback en app.py linea 29 (from PIL import Image) y PIL/Image.py linea 95. DIAGNOSTICO: la carpeta _internal/PIL del paquete estaba LITERALMENTE VACIA. Los ~76 modulos Python de Pillow SI viajaban, dentro del PYZ, pero ninguna de sus 8 extensiones en C, asi que Image.py se cargaba y moria al importar _imaging. CAUSA: Pillow importa su extension dentro de un try/except ImportError ('from . import _imaging as core') y el analisis estatico de PyInstaller no la arrastra. Descartadas dos hipotesis con dato: UPX estaba en True en el .spec pero NO esta instalado, asi que PyInstaller lo omitia y no era la causa; y no era la maquina del auditor, el paquete estaba incompleto y habria fallado en cualquier equipo. Tambien se comprobo que collect_dynamic_libs('PIL') devuelve 0: solo busca .dll, no .pyd, porque las extensiones de Python se supone que las descubre el grafo de imports. TRES CORRECCIONES. (1) El .spec pide Pillow explicitamente con collect_submodules('PIL'), que si lista PIL._imaging y las otras 7, con el porque documentado en el propio .spec para que nadie lo quite. (2) upx pasa de True a False en EXE y COLLECT: no hacia nada aqui porque no esta instalado, pero dejarlo en True significa que el binario saldria distinto segun la maquina donde se compile, y UPX es causa conocida de extensiones C que dejan de cargar y de falsos positivos de antivirus. (3) LO IMPORTANTE, nuevo subcomando 'autocomprobar' en main.py que importa DE VERDAD las 10 piezas criticas de arranque -la interfaz, PIL.Image, PIL._imaging, customtkinter, pandas, duckdb, pyarrow.parquet, xlsxwriter, openpyxl y playwright- y reporta todas las que falten, no solo la primera; build_release lo ejecuta sobre el .exe recien compilado y SE NIEGA A CREAR EL ZIP si algo falta. CAUSA DE QUE SE NOS ESCAPARA: la verificacion del 2026-09-10 fue insuficiente. Se comprobo 'cpa-salida --help' y los simbolos del PYZ, pero --help NO toca la interfaz, asi que nunca se importo app.py ni Pillow y la prueba paso por al lado. VERIFICADO: el build nuevo trae 8 extensiones de Pillow y la autocomprobacion corre dentro del propio build con las 10 piezas OK; PRUEBA NEGATIVA apartando los 6 _imaging*.pyd del paquete -reproduce EXACTAMENTE 'cannot import name _imaging from PIL' y la autocomprobacion sale con codigo 1 y dice 'paquete incompleto'-, con restauracion verificada; y el zip 1.0_20260911 (121 MB, 2509 archivos) lleva las extensiones, el .exe y _internal, sin datos de uso ni sesion del portal.

**Archivos:** `AutomationCostos.spec`, `main.py`, `scripts/build_release.py`

---

### 2026-09-11 07:56:33 — [CÓDIGO] Manual de uso en Word para el auditor, con huecos para capturas

Peticion de Oscar: un manual sencillo para repartir con el ejecutable, con espacio para pegar imagenes de la herramienta. Nuevo scripts/generar_manual_usuario.py -> Manual_Automation_Costos.docx, siguiendo el patron que ya usaban generar_guia_validacion.py y generar_guia_columnas.py: el documento se GENERA, no se edita a mano, asi que cuando cambie la interfaz se regenera en vez de quedar desactualizado en el disco de alguien. ESCRITO PARA QUIEN NO SABE NADA del proyecto: 9 secciones numeradas -abrir, la pantalla, configurar Ajustes la primera vez, un proveedor de punta a punta, los cinco pasos, corregir el Compras y regenerar, varios proveedores por lotes, que archivos salen, y que hacer si algo falla-. Regla que se siguio: si una instruccion no se puede seguir con el raton, no va en el manual; hay una comprobacion automatica de que el texto NO contiene python, .venv, parquet, duckdb, subcomando, pyinstaller, repositorio, sql server ni playwright, revisando tambien dentro de las tablas. 10 MARCOS PARA CAPTURAS, cada uno una tabla de una celda con 6 a 12 lineas vacias dentro y, en gris, que se espera ver en esa imagen; en Word se hace clic dentro y se inserta la foto sin tocar el resto. Se incluyen los tres avisos que mas dudas generan y que salieron de esta semana: que no se saque el .exe de su carpeta porque necesita _internal; que la columna 'concepto' cambia el criterio del entregable en cuanto se clasifica UN renglon, para que nadie crea que el archivo salio vacio por error; y que ante 'Usuario o contrasena incorrectos' hay que preguntar antes de reintentar, porque la cuenta es compartida y los intentos fallidos la bloquean. VERIFICADO releyendo el .docx generado: 88 parrafos, 20 tablas -10 huecos, 5 avisos, 5 de contenido-, los 9 titulos en orden y cero jerga tecnica.

**Archivos:** `scripts/generar_manual_usuario.py`

---

### 2026-09-10 17:11:41 — [CÓDIGO] El build limpia y comprueba dist/ ANTES de llamar a PyInstaller

El build del 2026-09-10 murio con WinError 32 sobre dist/AutomationCostos/logs/salida_96008_20260910_153507.log. CAUSA: empaquetado, config.BASE_DIR es la carpeta del ejecutable, asi que la aplicacion escribe logs/ y outputs/ DENTRO de dist/AutomationCostos/; PyInstaller borra esa carpeta entera para reconstruirla y se topa con archivos en uso. No era el .exe abierto -tasklist no mostraba ninguno-: era una COLA lanzada desde el .exe que seguia generando el entregable del proveedor 96008 y cuyo proceso hijo tenia el log abierto. LO GRAVE es cuando falla: PyInstaller ya habia empezado a borrar dist/AutomationCostos ('Removing dir') antes de toparse con el bloqueo, y un build que se cae a la mitad deja la instalacion inservible -paso el 2026-08-25, quedo sin los temas de customtkinter y el .exe ya no arrancaba-. Esta vez _internal sobrevivio por poco. HALLAZGO ADICIONAL: la comprobacion de 'el .exe no esta corriendo antes de compilar' que la entrada del 2026-08-25 dice que se añadio NO ESTABA en el codigo; se perdio o nunca se guardo. Verificado con git: build_release.py solo traia el cambio de NO_DISTRIBUIR. Se implementa ahora de verdad. NUEVO preparar_dist(), que corre al principio de compilar(): (1) comprueba que la aplicacion no este abierta intentando abrir el .exe en modo append, que es lo que Windows bloquea mientras se ejecuta; (2) borra logs/ y outputs/ de la carpeta del ejecutable -no son parte del build, no viajan en el zip por NO_DISTRIBUIR y PyInstaller los borraria igual, pero asi se borran ANTES, cuando todavia se puede abortar sin daño-; (3) si algo esta bloqueado aborta SIN HABER TOCADO NADA, nombrando el archivo exacto y las dos causas probables (la aplicacion abierta, o una cola todavia generando), con el tasklist para comprobarlo. VERIFICADO con 4 casos sobre carpetas temporales: sin dist/ no hace nada; con datos de uso libres los limpia y NO toca el .exe ni _internal; con un log bloqueado aborta nombrando el archivo y deja el build intacto; y con el .exe en uso corta antes de compilar. BUILD REAL HECHO Y VERIFICADO SOBRE EL BINARIO: dist/AutomationCostos_1.0_20260910.zip (113.1 MB, 2293 archivos), el .exe despacha 'cpa-salida --help' sin abrir la interfaz con codigo 0, el zip no lleva ni un rastro de logs/outputs/sesion, y se inspecciono el PYZ del binario: cpa_vision 6/6 simbolos nuevos, ui 2/2, app 3/3. Se borraron los 4 zips anteriores (479 MB) porque X: esta al 95%.

**Archivos:** `scripts/build_release.py`

---

### 2026-09-10 16:16:09 — [CÓDIGO] El acceso rechazado por CPA Vision se detecta al instante y no se reintenta

Tras resolver el 403, la corrida de Oscar llego HASTA EL LOGIN y fallo con 'Locator.wait_for: Timeout 60000ms exceeded' a los 2 min. La captura que ahora se guarda -logs/cpavision_login_error.png- lo resolvio de un vistazo: el portal mostraba 'Usuario o contrasena incorrectos'. O sea, la automatizacion funciono entera (cargo el portal, espero el formulario, escribio correo y contrasena, selecciono FacReview y pulso Iniciar sesion) y el portal rechazo las credenciales. EL DEFECTO: _login solo esperaba el EXITO -la URL de empresas y, de respaldo, su texto-, asi que una contrasena mal escrita costaba los dos tiempos de espera completos y salia como un timeout opaco, cuando el portal contesta al instante y con todas las letras. Nuevo _esperar_resultado_login() que vigila los DOS desenlaces a la vez en un bucle corto y se queda con el primero; se conservan intactas las dos señales de exito que ya existian. Nueva excepcion CredencialesInvalidas, hermana de SolicitudSinValores: respuesta definitiva que NO se reintenta. Eso ultimo importa por seguridad operativa mas que por tiempo: la cuenta de CPA Vision es COMPARTIDA y una tanda de 50 proveedores con dos intentos cada uno son 100 accesos fallidos seguidos, que es como se bloquea la cuenta para todo el equipo; por eso se atrapa ANTES del except generico del bucle de intentos y corta el lote entero -todos los proveedores usan las mismas credenciales, seguir no tiene sentido-. DETALLE DE IMPLEMENTACION que costo dos intentos: la deteccion mira el TEXTO del recuadro .okta-form-infobox-error, no si esta visible. El recuadro es parte del formulario de Okta y nace VACIO; el portal le escribe el mensaje solo al fallar, asi que 'tiene texto' es la señal exacta. Se usa text_content() -lee el DOM aunque el elemento este oculto- guardado tras un count(), porque un locator sin coincidencias haria esperar el timeout completo en cada vuelta del bucle. VERIFICADO con 5 casos, y el primero es el HTML REAL que guardo la corrida fallida: se reconoce en 0.22 s en vez de 2 min y con el mensaje del portal; exito por texto y exito por URL siguen funcionando; una pagina neutra y un formulario limpio con el recuadro vacio se agotan y avisan con la URL, o sea no hay falso positivo. HALLAZGO DE PASO sobre como probar estas cosas: el HTML guardado hay que cargarlo SIN JavaScript, porque al recargarlo con JS el widget de Okta re-renderiza el formulario y BORRA el recuadro -comprobado: con JS el elemento ni existe-.

**Archivos:** `automation_costos/cpa_vision.py`

---

### 2026-09-10 15:23:15 — [BUG] CPA Vision devolvia 403 a la descarga sin ventana: era el User-Agent

CAUSA RAIZ ENCONTRADA Y MEDIDA. La corrida de Oscar del 2026-09-10 fallo de nuevo sin ventana, pero esta vez la espera nueva del formulario dio el dato que faltaba: el titulo de la pagina era '403 Forbidden'. No era que el formulario tardara en montarse: el portal RECHAZABA la peticion antes de servir nada, y por eso no habia ni un solo input. MEDICION contra https://cpavision.mx/ sin credenciales, pidiendo solo la pagina publica de acceso: (1) sin ventana con el UA por omision 'HeadlessChrome/152.0.0.0' -> HTTP 403, titulo '403 Forbidden'; (2) sin ventana con el UA sin la marca Headless -> HTTP 200, titulo 'Login Cpa Vision'; (3) sin ventana con UA limpio MAS --disable-blink-features=AutomationControlled -> 200, identico al anterior; (4) con ventana como control -> 200, y su UA ya viene limpio. O sea: el UNICO factor es la cadena del User-Agent. El filtro del portal mira la cadena, no navigator.webdriver, asi que NO se agrega la bandera de ocultar automatizacion: no aporta nada y seria ruido. Esto explica por que las 212 h de descargas historicas nunca lo vieron: todas se corrieron con ventana, y el default headless=1 es del 2026-08-24. SOLUCION: nuevo _user_agent_sin_marca_headless(), que LEE el UA del propio navegador sobre about:blank -sin red- y le quita la marca (HeadlessChrome/ -> Chrome/, HeadlessEdg/ -> Edg/, y una limpieza final). Se lee en vez de escribirse a mano porque una cadena fija envejece con cada version de Edge y volveria a delatarnos. Solo se aplica SIN ventana: con ventana no se toca absolutamente nada, que es el camino con historico. De paso se extrajo _nuevo_contexto(browser, settings), porque la construccion del contexto estaba repetida IDENTICA en los CINCO sitios que abren navegador; con esa duplicacion, una correccion como esta habria entrado en unos caminos y no en otros. VERIFICADO contra el portal real con el codigo de produccion: sin ventana da 200 y 4 campos visibles tras la espera, con ventana igual; y con dobles: con ventana no se pone user_agent ni se crea contexto extra, sin ventana con UA delator se sanea, sin ventana con UA ya limpio no se inventa nada, la variante HeadlessEdg tambien se corrige y el storage_state se sigue reutilizando. IMPORTANTE: los DOS arreglos de esta sesion eran necesarios y no uno u otro -medido que justo despues del domcontentloaded hay 0 campos visibles INCLUSO con ventana, asi que sin la espera del formulario el login seguiria siendo una carrera-. Compila, CLI intacto y los 5 caminos comparten ya el mismo constructor de contexto.

**Archivos:** `automation_costos/cpa_vision.py`

---

### 2026-09-10 14:49:35 — [CÓDIGO] La contrasena de CPA Vision se puede ver manteniendo pulsado el ojo

Peticion de Oscar: en Ajustes no habia forma de comprobar que la contrasena quedo bien tecleada, y una credencial mal escrita solo se descubre cuando el lote falla contra el portal minutos despues. Nueva fabrica ui.campo_secreto(): el campo de siempre mas un ojo que revela el texto MIENTRAS SE MANTIENE PULSADO. Es a proposito un gesto sostenido y NO un interruptor: un interruptor deja la contrasena a la vista hasta que alguien se acuerde de apagarlo, y aqui la cuenta de CPA Vision es compartida (hector.saucedo@prgx.com) y la pantalla se comparte en reuniones; manteniendolo pulsado el descuido es imposible. Tambien se oculta si el puntero SALE del ojo sin soltar, para que arrastrar fuera no deje el campo destapado. DOS DECISIONES DE IMPLEMENTACION que responden a restricciones ya documentadas del proyecto: (1) es una CTkLabel y no un CTkButton, porque cada control con esquinas redondeadas es un CTkCanvas mas y el presupuesto de recursos de ui.py existe porque esto corre en equipos de un solo nucleo; una etiqueta con cursor de mano se ve igual y no cuesta canvas. (2) el glifo es U+25C9 y no el emoji de ojo U+1F441: Tk en Windows no dibuja de forma fiable lo que pasa de U+FFFF, y toda la interfaz ya se mantiene dentro del BMP (v, >, reloj, prohibido). Se agrego una pista en letra chica bajo la fila explicando el gesto, porque un icono solo no lo comunica. La logica vive en ui.py y app.py solo la cablea, que es la separacion que el modulo declara. VERIFICADO sobre widgets reales, no sobre el codigo: nace oculta, pulsar revela, soltar oculta, salir sin soltar oculta y el valor no se toca, en los DOS temas; la vista Ajustes real construye con 6 campos de los cuales exactamente uno esta oculto y un solo ojo, y ese ojo cableado revela y oculta; y aguanta DOS vueltas de cambio de tema -que es donde fallaba el bug historico de widgets muertos- conservando la contrasena capturada. Nota para futuras pruebas: event_generate no entrega eventos a widgets sin mapear, y CTkLabel.bind reenvia a su canvas y su label internos, no al frame.

**Archivos:** `automation_costos/ui.py`, `automation_costos/app.py`

---

### 2026-09-10 14:39:55 — [DECISIÓN] No se hara actualizacion parcial de la Validacion de un proveedor gigante

Oscar pregunto si la GUI podia aceptar varios Compras de un gigante (anios, trimestres o meses) y regenerar SOLO lo que se suba. Se analizo y se DESCARTA la version parcial, por correccion y no por costo: las devoluciones MR8M/KG-14 se aplican sobre el Consolidado COMPLETO y una factura MR8M que cae en folios de varios trimestres REPARTE EN CASCADA entre ellos (LOGICA_NEGOCIO 11); recalcular un trimestre aislado le daria un reparto distinto, asi que el resultado dependeria de que archivos se subieron. Son cifras mal que no avisan, que es el peor tipo de error para un entregable que se le manda al proveedor. Se suma que ~0.3% de los folios cruzan frontera de trimestre, asi que el corte no es limpio. La version CORRECTA si es posible -seleccionar todos los Compras del proveedor y reconstruir la Validacion entera con write_validation_streaming, que ya recibe los trozos de uno en uno, la misma maquina que saco Arca y Pepsico- pero leer los 24 archivos de Arca con openpyxl (~7 GB) se lleva horas. DECISION DE OSCAR (2026-09-10): OMITIRLO POR AHORA. Cuando un gigante necesite rec\u00e1lculo se hace como se hizo Arca con Andrea: un script que recalcula desde SQL, determinista y reproducible, en vez de editar 24 Excel a mano -que ademas es lo que ya se hizo, porque esos Compras ni siquiera son homogeneos entre si-. Queda cubierto el riesgo: el guardarrail que se agrego hoy a app._destino_validacion hace que un Compras con sufijo de anio o trimestre NO se considere canonico, asi que nadie puede pisar el entregable bueno con una Validacion parcial desde la interfaz.

**Archivos:** `automation_costos/app.py`

---

### 2026-09-10 14:35:39 — [CÓDIGO] CPA Vision sin ventana: el fallo de login ya deja evidencia y se espera al formulario

Lo reporto Oscar probando la cola desde la GUI con 'Descargar sin ventana' marcada: 'ERROR: No se encontro el campo requerido en CPA Vision: usuario/correo', a los ~30 s, sin generar nada. DOS HALLAZGOS. 1) NO ES UN PROBLEMA DE SELECTORES: el cuarto y ultimo recurso de _fill_first_visible es page.locator('input').nth(0), o sea CUALQUIER campo de texto de la pagina; para que tambien falle, la pagina tuvo que quedarse sin un solo input visible durante los 20 s que suman los cuatro intentos de 5 s. El formulario de acceso simplemente no estaba. Causa mas probable: page.goto(wait_until='domcontentloaded') vuelve en cuanto llega el HTML y el portal monta la pantalla de acceso con JavaScript despues. 2) ESTABAMOS CIEGOS, y esto era lo mas grave: el login del lote ocurre en _start_authenticated_downloads_session, que NO tenia try/except; todos los _save_debug_artifacts viven dentro del bucle de proveedores, que aun no habia empezado, y el except de request_vendor_master_batch recibe page=None porque la funcion nunca llego a devolverla. Por eso no habia ni captura ni HTML de la corrida -verificado en dist/AutomationCostos/logs, el artefacto mas reciente era del 15-ago- y no era falta de espacio: X: tiene 82 GB libres. TRES CAMBIOS, todos ADITIVOS y sin tocar la mecanica del portal, que esta documentada como resuelta: (a) _start_authenticated_downloads_session envuelve navegacion+login+descargas, guarda cpavision_login_error.png/.html, anota la URL y el titulo reales, cierra el navegador y propaga -cubre tambien los reintentos, porque _restart_ delega en ella-; (b) nuevo _esperar_formulario_login espera hasta timeout_ms (60 s, el triple que antes) a que haya cualquier campo de texto visible, a proposito por 'cualquiera' y no por uno concreto: es la senal minima de que la pantalla se dibujo y no se casa con la estructura del portal, que no controlamos; su error dice donde estaba el navegador y que revise la captura; (c) nuevo _donde_esta(page) devuelve url y titulo sin lanzar nunca, y se avisa explicitamente cuando la sesion guardada ya no es valida, que es un caso normal y hoy se leia como fallo tecnico. VERIFICADO con dobles, sin abrir un navegador: formulario presente sigue de largo, ausente lanza con URL/tiempo/pista, _donde_esta tolera una pagina muerta, el fallo de login deja captura + cierra + propaga sin guardar sesion, y el camino feliz queda identico -sin captura, sin cierre, con storage_state-. PENDIENTE: con la captura ya en disco, la siguiente corrida sin ventana dira si era la carga tardia o si el portal se comporta distinto sin ventana. Recordatorio de la bitacora del 24-ago: las 212 h de descargas historicas se corrieron TODAS con ventana.

**Archivos:** `automation_costos/cpa_vision.py`

---

### 2026-09-10 14:35:08 — [CÓDIGO] La Validacion generada desde la GUI ya actualiza el reporte consolidado

Peticion de Oscar: lo que se ejecuta desde la interfaz tiene que quedar mapeado en el reporte de control, igual que por terminal. CAUSA REAL, y no era una prohibicion de diseno: el enganche actualizar_desde_validacion ya existia -su propio docstring dice que es para 'main.py validate o el boton de la GUI'- pero solo actua si el archivo tiene la forma canonica raiz/<numero>_<nombre>/Validacion_<numero>_<nombre>.xlsx, y la interfaz escribia siempre suelto: <stem>_Validacion_Condiciones.xlsx en la raiz de la carpeta de salida. Con ese nombre nunca podia enganchar. Ademas todo el flujo paso a paso de la GUI (Compras, _EDI, _Recalculado) cae en la raiz, no en carpetas de proveedor, asi que no bastaba con llamar a la funcion. SOLUCION: nueva app.base_de_compras() reconstruye el <numero>_<nombre> desde el nombre del Compras recorriendo al reves la convencion de pipeline._nombre_base, quitando los sufijos que la propia interfaz encadena (_EDI, _Recalculado, en cualquier orden y repetidos); _destino_validacion() decide entre la ruta canonica y la suelta de siempre. DOS GUARDARRAILES: (1) si el nombre trae sufijo de anio o trimestre (_2020, _2020-T3) NO se considera canonico, porque es un trozo de un proveedor grande y escribir su Validacion parcial encima del entregable bueno seria un error que no se nota; (2) sobrescribir un entregable existente pide confirmacion, porque es el archivo que ya se le pudo haber mandado al proveedor. Si el Compras ya vive en la carpeta de su proveedor, esa manda sobre la carpeta de Ajustes. Cuando no se puede deducir el proveedor se conserva EXACTAMENTE el comportamiento anterior (archivo suelto, sin tocar el reporte) y se dice en la bitacora, porque es preferible no actualizar a actualizar con un numero parcial. El reporte se importa dentro de la funcion para no pagar pandas en el arranque -equipos de un solo nucleo- y nunca puede tumbar el paso: la Validacion ya esta en disco. VERIFICADO: 10 casos de base_de_compras incluidos los encadenados y los trozos de gigante; los 4 casos de _destino_validacion sobre la instancia real de la ventana, que ademas construye y cierra; y el enganche del reporte con dobles -el nombre viejo NO lo toca, el nuevo si y apunta a la raiz correcta, la variante _<anio> dentro de su carpeta sigue enganchando y una validacion en carpeta ajena sigue sin sembrar reportes-. CLI intacto: 17 subcomandos y validate sin cambios.

**Archivos:** `automation_costos/app.py`

---

### 2026-08-27 10:02:13 — [CÓDIGO] Compras: formato numerico de dos decimales (peticion de Monica)

Los valores se escribian en formato General y se veian como 21.000000000, 345.120000000 y 2E+06. Se aplica formato POR COLUMNA en _write_compras_sheet (set_column, no celda por celda, para no romper el modo constant_memory de xlsxwriter en proveedores grandes): montos y costos #,##0.00; tasas y factores konv 0.###### (un IEPS de 0.265 NO se puede redondear a 0.27); cantidades #,##0.##; folios numericos (vndnbr, ponbr, rcvnbr, strnbr, dptnbr) formato 0 para matar la notacion cientifica. El valor guardado no cambia, solo la presentacion. Los formatos se crean una vez por libro y se reusan entre hojas.

---

### 2026-08-26 19:32:27 — [CÓDIGO] Arca recalculado con las formulas de Andrea (caso aislado) + hallazgo del pago inflado

Pedido de Andrea Soler (reunion 2026-08-26): recalcular el 100% de Arca 391250 con su metodologia y entregar Validaciones por ANIO. Nuevo scripts/arca_recalculo_andrea.py, AISLADO: el pipeline normal no se toco. Su correccion reconstruye el costo unitario desde la compra bruta menos el descuento comercial de 2.5 por ciento que Arca da a pie de factura, dividida entre la cantidad REDONDEADA (can_rec trae 79.92 donde son 80 piezas). El reemplazo es 1 a 1 con la formula del programa: imp_aud = cto_aud x can_rec x impuestos pasa a Importe Audi2 = Mejor Costo 2 x Canti Correc x impuestos, y Mejor Costo 2 conserva la estructura de cto_aud solo que el costo del sistema pasa de ctouni a Costo Unitario VF. La Validacion sale con la metodologia nueva SIN tocar el codigo probado: se sobrescriben las cuatro columnas que build_consolidado y build_detalle_rapido leen por nombre (can_rec, ctouni, cto_aud, imp_aud), asi los encabezados quedan identicos. Se recalculo desde SQL y no se leyeron los Compras ya entregados porque NO son homogeneos: el 2020-T1 tiene 112 columnas, los T2-T4 de 2020 tienen 106 y de 2021 en adelante 105, se les fueron agregando columnas a mano. VERIFICADO contra el archivo de Andrea (10,293 renglones reales, valores calculados por Excel): las 15 columnas coinciden, peor diferencia 0.00000000 salvo los importes con 0.00005 por el redondeo a 4 decimales. El layout quedo ESPEJO: 122 columnas en las mismas letras que su archivo, asi sus formulas copiadas siguen apuntando bien; para eso hubo que replicar que ella renombro concaten a 'Cruce con diferencias' y la MOVIO junto a rcvnbr. RESULTADO: 24 Compras + 6 Validaciones en 'Arca regenerado recalculos', 11,538,841 renglones, 419,295 folios, 84,423 con diferencia, total 48,254,618.44. HALLAZGO CRITICO: 1,355 folios (1.6 por ciento) aportan 44,977,428 (93.2 por ciento del total) y su tot_pagado_ne NO cuadra con el importe del cheque. Caso revisado: folio de 27 renglones, UN cheque y UNA factura, paynetamt 110,002.33 contra tot_pagado_ne 330,006.43, exactamente el triple, mientras la suma de Importe Audi2 da 110,002.31 (la diferencia real es de 2 centavos). Viene asi desde F_COMPRAS, el pipeline solo lo toma con max por folio; es el problema que Andrea ya conocia ('todavia viene mal el tema de tot netamon'). Sin esos folios el total seria 3,277,190.33. NO se corrigen ni se excluyen por decision de Oscar: se MARCAN con tres columnas al final del Consolidado (Importe del Cheque, Pago NE / Cheque, Alerta Pago) y el Resumen dice cuantos folios y cuanto monto estan marcados. Usar paynetamt en vez de tot_pagado_ne NO es la solucion: daria MAS diferencias (88.3M contra 76.9M) porque en el 99.7 por ciento de los folios tot_pagado_ne esta bien. Otras dos verificaciones: los 3,472,571 renglones con cantidad 0 (30 por ciento) son inofensivos, CERO de ellos tiene compra_bruta distinta de 0, son los renglones sin nota de entrada ya documentados; y las devoluciones MR8M/KG-14 se aplicaron (Arca tiene 100,019). Dos tropiezos resueltos: pyarrow abortaba por tipos mezclados (canfac_edi trae el numero 0 y el texto '0'), mismo HALLAZGO 3 del caso Arca anterior, resuelto con una normalizacion que decide POR CONTENIDO y no por lista fija porque aqui se guardan 122 columnas y la lista de aquel caso cubria 9, protegiendo aparte los codigos que parecen numeros; y el cache dejaba fuera folio y Trimestre al pasar al orden espejo, lo que habria reventado la fase 2 tras hora y media de extraccion.

---

### 2026-08-25 17:22:41 — [CÓDIGO] El intervalo que no cabe en memoria se parte solo: trimestre -> mes -> dia

Requisito de Oscar: el .exe NO puede fallar por memoria. Hasta hoy los tres caminos por intervalos (generar_salida_proveedor_por_anios, generar_validacion_grande, generar_compras_grande) fijaban la granularidad de antemano —trimestre, o mes con --por-mes— y si un intervalo no cabia, la corrida entera moria. Cuanto cabe en RAM no se puede saber de antemano: depende de la maquina del auditor, de lo que tenga abierto y de cuantos CFDI traiga ese trimestre. Ahora los tres comparten _recorrer_intervalos, que intenta el trimestre y, si revienta con MemoryError, lo reintenta partido en meses; si un mes tampoco cabe, en dias; si un dia tampoco, corta con un mensaje util en vez de recursar sin fin. El proveedor sale igual, solo repartido en mas archivos. RIESGO CUBIERTO (lo importante): un reintento sobre un intento a medias podia contar los importes DOS VECES, que es peor que el MemoryError porque no se nota. Se cerro por dos lados: (1) los acumuladores por folio/factura se calculan en temporales y se reasignan solo cuando los dos estan listos, asi un fallo a medio camino deja el acumulado intacto; (2) cada llamador pasa un `deshacer` que retira lo que el intento fallido si alcanzo a registrar (su metrica de cruce, su intervalo, su pickle). Ademas la pasada 2 ahora guarda el pickle del detalle ANTES de escribir el Excel, para que un reintento no deje un Compras del trimestre conviviendo con los Compras de sus meses. Verificado: (a) motor con 6 casos —subdivision de un nivel a la vez, corte de la recursion en el dia, sustitucion del intervalo fallido, y el deshacer; (b) equivalencia end-to-end con Selecta 741 2024 saboteando un trimestre en la pasada 1 y otro en la pasada 2: folio_global e inv_global identicos tabla contra tabla en los tres escenarios (1,846 folios, 125,314,295.78 de debio pagar) y la Validacion con el mismo Resumen, Consolidado y Detalle. Se agrego --por-mes a cpa-salida para forzar la granularidad fina de antemano en una maquina que ya se sabe justa.

---

### 2026-08-25 16:02:34 — [CÓDIGO] La Validacion del camino por trimestres se escribe en streaming (arregla MemoryError)

generar_salida_proveedor_por_anios acumulaba las filas de los folios con diferencia en memoria (trozo.copy() con las 105 columnas del Compras) y las pasaba a write_validation_from_dataframe, que arma el libro con openpyxl: los ~10M objetos Cell de la hoja 'Detalle PAGOS' viven todos a la vez. PROPIMEX 77305 (2025-01-01..2026-03-31, 1,285,317 renglones -> 472,257 de folios con diferencia) murio ahi con MemoryError, con los cinco Compras ya escritos y ~35 min tirados. El escritor correcto ya existia en el mismo modulo y estaba hasta importado: write_validation_streaming (xlsxwriter en constant_memory), pero solo lo usaba el camino de los gigantes. Ahora cada trimestre se recorta a _COLUMNAS_FUENTE, se deja en un pickle temporal, y la Validacion se escribe leyendo los trozos de uno en uno; el periodo se calcula acumulando los anios de cada trozo. Verificado antes de re-lanzar con 475,000 renglones sinteticos: 50.8 MB en 174 s, libro con Resumen/Consolidado/Detalle PAGOS completos. Ademas se agrego el flag --reanudar a cpa-salida: reusa los Compras_*.xlsx que ya esten en disco en vez de reescribirlos (el re-cruce es determinista), para retomar un proveedor grande que murio despues de escribirlos sin pagar otra vez los ~7 min por trimestre. OJO: el .exe de dist/ NO tiene este arreglo ni los dos del 2026-08-24; hay que recompilar.

---

### 2026-08-25 12:52:52 — [CÓDIGO] Si el camino en memoria se queda sin RAM, se reintenta por trimestres

PROPIMEX (77305) fallo tras 6.7 min con numpy _ArrayMemoryError: 'Unable to allocate 569 MiB for an array with shape (58, 1285317)', dentro de cruzar(). CAUSA: el pipeline elige camino con MAX_FILAS_EN_MEMORIA = 2,500,000, un numero FIJO, y PROPIMEX tiene 1.28M — por debajo del umbral, asi que intento el camino en memoria. Pero que quepa o no depende de la RAM LIBRE, no del conteo: el equipo tiene 24 GB pero solo 11.6 estaban libres (la interfaz abierta y lo demas ocupaban la mitad), y el proceso llego a 10.2 GB. El umbral se habia calibrado con la maquina despejada ('Celaya 1.24M y Nestle ~1.3M caben'). En los equipos de los auditores, que tienen menos memoria, esto fallaria mucho mas seguido. SOLUCION: en vez de mover el umbral a ojo —que seguiria siendo un numero fijo y seguiria dependiendo de la maquina— se atrapa MemoryError en generar_salida_proveedor y se reintenta por el camino de trimestres, que no carga el periodo completo de una vez. Se llama gc.collect() antes de reintentar. El resultado es equivalente; solo cambia como se calcula. Verificado que numpy._core._exceptions._ArrayMemoryError es subclase de MemoryError, y probados los tres escenarios con dobles: sin memoria reintenta por trimestres, con memoria NO reintenta, y un gigante va por trimestres de entrada sin intentar memoria. Tambien: se añadio la comprobacion de que el .exe no este corriendo ANTES de compilar. Un build lanzado con la aplicacion abierta borra _internal y se topa con un archivo bloqueado, dejando la instalacion inservible (le faltaban los temas de customtkinter y el .exe ya no arrancaba). Paso hoy.

---

### 2026-08-25 12:07:07 — [CÓDIGO] CPA Vision: el reintento reventaba cuando el portal ya tenia la solicitud en proceso

Lo reporto Oscar con captura: PROPIMEX (77305 / PRO840423SG8) fallo con 'Timeout 60000ms exceeded' tras 3.7 min, y la cola no genero nada. La captura de depuracion mostro el modal del portal: 'Solicitud fallida - Ya existe una solicitud identica en proceso ID: 659262 - CSVs'. RECONSTRUCCION: el intento 1 SI creo la solicitud 659262; algo fallo despues, el codigo reintento, y el reintento arranca desde cero y manda una solicitud IDENTICA, que el portal rechaza. CAUSA EXACTA: _wait_for_request_confirmation esperaba un texto con 'ID: NNNN' Y 'motivo' Y 'frecuencia', que son los campos del modal de EXITO. El modal de rechazo trae el ID pero no trae motivo ni frecuencia, asi que la espera se agotaba a los 60 s y el proveedor se marcaba como error — aunque su solicitud estuviera perfectamente creada y encolada en el portal. SOLUCION, aditiva: la espera ahora tambien reconoce el modal de duplicada; _submit_download_request extrae el ID que el portal reporta, lo reutiliza y sigue esperando ESE ZIP, saliendo antes de intentar llenar Motivo/Frecuencia, que ese modal no tiene. Es lo correcto porque 'identica' significa mismo RFC, mismos años y mismos archivos: el trabajo ya esta encolado y los datos son los mismos. Se añadio _cerrar_modal(), que nunca lanza. El camino de exito quedo intacto. Verificado: la regex reconoce el texto real de la captura, con acento y con saltos de linea, y NO coincide con el modal de exito ni con texto suelto; el JS que recibe el navegador se capturo y se valido. La cola, por su parte, se comporto bien: no genero el entregable porque la descarga fallo, que es justo la regla de listos_para_generar (evita entregar un Excel con el bloque EDI vacio y sin marcar error).

---

### 2026-08-25 10:51:11 — [CÓDIGO] GENERAR TODO ahora incluye la descarga, y Detener funciona dentro del pipeline

DOS COSAS. 1) GENERAR TODO hace el flujo completo: resolver RFC, descargar de CPA Vision, Compras, cruce EDI, recalculo y Validacion. Antes asumia el Parquet ya descargado. Tres decisiones: la descarga se OMITE si el Parquet ya cubre todos los años del periodo (repetirla cuesta ~8 min y el portal encola por usuario; se dice en la bitacora para que no parezca que se salto un paso); si el portal responde SolicitudSinValores se avisa y se CONTINUA hasta el entregable, porque 'Sin valores' es una respuesta definitiva y no un fallo —con el periodo por omision, que llega a 2026, es un caso de todos los dias y abortar dejaria al auditor sin entregable por un dato que no existe—; y si faltan años pero no hay credenciales se dice claro antes de empezar. Los tres caminos probados con dobles. 2) DETENER: lo reporto Oscar, no funcionaba. La cancelacion es cooperativa y solo se revisaba en tres puntos (esperando una descarga del portal, entre proveedores de un lote, y entre proveedores de la cola de generacion); DENTRO del pipeline no habia ninguno y generar_salida_proveedor ni siquiera aceptaba el parametro, asi que pulsar Detener durante Compras/cruce/Validacion levantaba la señal, apagaba el boton y no paraba nada. Se añadio cancelado a pipeline.generar_salida_proveedor y a pipeline_streaming.generar_salida_proveedor_por_anios, con cinco cortes en el camino en memoria (antes de SQL, del cruce, de la Validacion y del Compras) y dos en el camino de gigantes (al empezar cada pasada de cada trimestre, que es donde mas falta hacia porque son corridas de horas). Los cortes van SOLO entre paso y paso: abortar a media escritura dejaria un Excel corrupto que ademas parece valido, asi que Detener no es instantaneo y eso es deliberado. Compatible hacia atras: cancelado es opcional y por terminal no se pasa. Verificado: con la señal levantada corta antes de tocar SQL; sin señal el comportamiento es identico; 760/760 proveedores del plan siguen dando los mismos comandos y el CLI responde igual.

---

### 2026-08-25 10:22:43 — [CÓDIGO] El empaquetador ya no mete la sesion de CPA Vision en el zip

Empaquetado, config.BASE_DIR es la carpeta del ejecutable, asi que la aplicacion escribe logs/ y outputs/ justo al lado del .exe. build_release.empaquetar() comprimia TODO lo que colgara de dist/AutomationCostos/, asi que re-empaquetar despues de haber usado la aplicacion —y con --sin-compilar es exactamente lo que pasa— metia esas carpetas en el zip. No es solo basura: logs/cpavision_state.json es el estado de sesion de Playwright y contiene 42 COOKIES AUTENTICADAS de cpavision.mx y facreview.cpavision.mx. Repartir ese zip le habria entregado a cada auditor la sesion del portal de quien compilo, ademas de su cola de trabajo, sus rutas y las capturas de los errores. Se detecto al auditar el contenido del zip antes de entregarlo a los auditores. Se añadio NO_DISTRIBUIR = ('logs','outputs') y el empaquetador las salta reportando cuantos archivos omitio. El zip del 2026-08-25 ya construido estaba limpio por casualidad —se compilo a las 09:28, antes de la prueba de las 10:01—, pero cualquier rebuild posterior habria filtrado. Verificado re-empaquetando CON los datos de la prueba presentes: 8 archivos omitidos, 0 rastros de logs/outputs/sesion en el zip, y el .exe, los 2505 archivos de _internal y el README si van dentro.

---

### 2026-08-25 09:28:52 — [CÓDIGO] CRITICO: el .exe abria un dialogo modal al fallar y colgaba la tanda entera

Lo detecto Oscar con una captura de pantalla al probar el .exe: 'Unhandled exception in script - Failed to execute script main'. El .exe se compila sin consola (console=False) y en ese modo el bootloader de PyInstaller atiende cualquier excepcion no atrapada abriendo un CUADRO DE DIALOGO MODAL con el traceback. En un doble clic eso es util; como subproceso de la cola por lotes es un desastre: el proceso hijo se queda esperando un clic que nadie va a dar, y una tanda nocturna se cuelga ENTERA en el primer proveedor que falle. Justo el escenario para el que existe la cola. Explica ademas por que las dos pruebas anteriores tardaron minutos en vez de segundos: estaban esperando el clic. SOLUCION: no se desactivo disable_windowed_traceback (eso quitaria el aviso tambien para el arranque de la interfaz, donde si sirve). Se separo main() en dos: main() parsea y despacha la interfaz, y _despachar(args) corre el subcomando dentro de un try/except que escribe el traceback en stderr —o sea en el log del proveedor, que es donde se busca— y sale con SystemExit(1), que es lo que ejecutor.ejecutar ya sabe interpretar. Asi un subcomando NUNCA deja escapar una excepcion y el dialogo se conserva solo para la GUI. VERIFICADO sobre el .exe recompilado: termina SOLO en 10 segundos (antes se colgaba), codigo de salida 1, y el log sale en orden correcto y con los acentos bien: primero [0/4] y [1/4], despues el traceback, y 'no devolvio compras' con la o acentuada intacta. Las tres correcciones de esta sesion (auto-invocacion, UTF-8 + line buffering, y este dialogo) quedan verificadas sobre el binario, no sobre el codigo fuente.

---

### 2026-08-25 09:18:07 — [CÓDIGO] Logs del .exe: UTF-8 y linea a linea (dos defectos que destapo la prueba real)

Probar el .exe compilado contra un proveedor inexistente confirmo lo importante —entra al pipeline de verdad, propaga codigo de salida 1, y tanto los print() como el traceback llegan al log redirigido— pero destapo dos defectos en el log, que es lo unico que queda cuando un proveedor falla de madrugada. 1) CODIFICACION: el hijo escribia en la ANSI de Windows (cp1252) mientras el log se lee como UTF-8, asi que 'no devolvio compras' salia como 'no devolvi? compras'. Los mensajes del pipeline son en español: es casi cada linea. 2) BUFFER: redirigido a un archivo, stdout usa buffer de bloque y se vacia al terminar, DESPUES de que stderr ya escribio el traceback; el log quedaba al reves, con el error arriba y los pasos que llevaron a el abajo. Ambos se corrigen en _asegurar_salida_estandar(), que ahora no solo cubre el caso de sys.stdout en None sino que reconfigura los flujos existentes a encoding utf-8, errors replace y line_buffering. Se recompilo el .exe (AutomationCostos_1.0_20260825.zip) para verificarlo sobre el binario real, no sobre el codigo fuente.

---

### 2026-08-24 23:50:53 — [CÓDIGO] La cola por lotes ya genera entregables desde el .exe (el ejecutable se auto-invoca)

PROBLEMA: la fase de generacion de la cola lanza main.py como subproceso con el python.exe del .venv. Dentro del .exe empaquetado no existe ni ese interprete ni main.py, asi que desde el ejecutable la cola solo podia descargar. Oscar lo necesita para repartir el aplicativo. SOLUCION: el ejecutable se auto-invoca. El punto de entrada del .exe ES main.py, que ya despachaba subcomandos (sin subcomando abre la GUI, con subcomando corre el CLI), asi que 'AutomationCostos.exe cpa-salida --vendor ...' ejecuta exactamente el mismo codigo que 'python main.py cpa-salida ...'. En ejecutor.py se añadio CONGELADO (sys.frozen), lanzador() —que devuelve [sys.executable] empaquetado y [python, 'main.py'] sin empaquetar— y directorio_trabajo(), porque empaquetado RAIZ apunta dentro de _internal y no sirve como cwd; se usa la carpeta del ejecutable. El guardarrail del interprete ahora solo aplica sin empaquetar. SEGUNDO PROBLEMA, el que habria roto todo en silencio: el .exe se compila con console=False y en ese modo PyInstaller puede dejar sys.stdout y sys.stderr en None; como la cola le entrega al hijo un archivo de log en los descriptores 1 y 2, el primer print() del pipeline habria reventado con AttributeError y el proveedor se habria marcado como fallido sin que el log dijera por que. Se añadio _asegurar_salida_estandar() en main.py, que reengancha los flujos a los descriptores que el padre entrego (o a devnull si no hay ninguno). VERIFICADO SOBRE EL .EXE YA COMPILADO: despacha 'cpa-salida --help' sin abrir la GUI, con codigo 0, y escribe 1260 bytes en el stdout redirigido, que es la prueba de que el blindaje funciona en modo sin consola. Sin empaquetar nada cambia: 760/760 proveedores del plan siguen dando comandos identicos con el lanzador de siempre.
